...
const DEFAULT_TERMINAL_COLORS: Record<"light" | "dark", TerminalColors> = {
  light: {
    ...
    foreground: "#1a1c20",
    cursor: "#2a7a5a",
    selectionBackground: withAlpha("#1a1c20" as HexColor, 0.14),
  },
  dark: {
    background: "#0d1117",
    foreground: "#c8c5be",
    cursor: "#58e8aa",
    selectionBackground: withAlpha("#58e8aa" as HexColor, 0.18),
  },
}
...
export const Terminal = (props: TerminalProps) => {
  ...
  const getTerminalColors = (): TerminalColors => {
    ...
    const alpha = mode === "dark" ? 0.22 : 0.16
    const base = text.startsWith("#") ? (text as HexColor) : (fallback.foreground as HexColor)
    const selectionBackground = withAlpha(base, alpha)

    // Use the theme's interactive accent for the cursor instead of text color
    const cursorRaw = resolved["border-interactive-base"] ?? resolved["border-selected"]
    const cursor =
      cursorRaw && cursorRaw.startsWith("#") ? (cursorRaw as HexColor) : fallback.cursor
