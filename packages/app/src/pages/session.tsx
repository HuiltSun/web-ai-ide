...
import { useEditor, EditorProvider } from "@/context/editor"
...
export default function Page() {
  ...
  const isIdeLayout = createMemo(() => isDesktop() && layout.editor.opened())
  const size = createSizing()
  const desktopReviewOpen = createMemo(() => isDesktop() && view().reviewPanel.opened())
  const desktopFileTreeOpen = createMemo(() => isDesktop() && layout.fileTree.opened())
  const desktopSidePanelOpen = createMemo(() => desktopReviewOpen() || desktopFileTreeOpen())
  const sessionPanelWidth = createMemo(() => {
    if (isIdeLayout()) return `${layout.session.width()}px`
      ...
      <div class="flex-1 min-h-0 flex flex-col md:flex-row">
        ...
        <Show when={isIdeLayout()}>
          <FileTreePanel
            ...
            onFileDoubleClick={(path) => editor.open(path)}
            onFileClickAllTab={(path) => editor.open(path)}
            sizeActive={() => size.active()}
            onSizeStart={() => size.start()}
            onSizeTouch={() => size.touch()}
          />
          <EditorPanel />
        </Show>
        ...
        >
          ...
          <Show when={desktopSidePanelOpen() || isIdeLayout()}>
            <div onPointerDown={() => size.start()}>
              <ResizeHandle
                direction="horizontal"
                edge={isIdeLayout() ? "start" : "end"}
