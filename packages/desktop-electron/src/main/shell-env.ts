...
export function loadShellEnv(shell: string) {
  ...
  const cached = store.get("shellEnv") as { shell: string; env: Record<string, string> } | undefined
  if (cached && cached.shell === shell) {
    console.log(`[server] Using cached shell environment (${Object.keys(cached.env).length} vars)`)
    return cached.env
  }
  ...
  if (interactive.type === "Loaded") {
    ...
    store.set("shellEnv", { shell, env: interactive.value })
  ...
  if (login.type === "Loaded") {
    ...
    store.set("shellEnv", { shell, env: login.value })
    return login.value
  }
