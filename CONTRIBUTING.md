...
- Core pieces:
  ...
  - `packages/ui`: The shared UI component library (Markdown rendering, diff viewer, CodeMirror, themes, i18n)
  - `packages/core`: Shared core utilities (file system, npm tools, Effect runtime, logging)
  - `packages/desktop`: The native desktop app, built with Tauri (wraps `packages/app`)
  - `packages/desktop-electron`: The Electron desktop app (wraps `packages/app`)
  - `packages/plugin`: Source for `@opencode-ai/plugin`
  - `packages/sdk/js`: TypeScript SDK for programmatic access
  - `packages/web`: Documentation website (Astro + Starlight)
  - `packages/docs`: Documentation source content (Mintlify)
